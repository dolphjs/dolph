...
:::tip
When you define the `mongo`, `typeorm`, or `sequelize` keys in your configuration, the `DolphFactory` will automatically attempt to connect to these databases before starting the HTTP server. This saves you from writing manual connection boilerplate!
:::

**Strictly Typed Configuration:**
Under the hood, the `dolph_config.yaml` values map directly to the official configuration interfaces provided by the underlying ORMs. For TypeORM, the `options` block maps perfectly to `DataSourceOptions`. For Sequelize, the `options` block maps to Sequelize's native `Options` interface. This ensures full auto-completion and seamless support for advanced database configurations right out of the box!

**Retrieving Database Instances:**
Once auto-initialized, you can instantly grab your database clients anywhere in your application:
...
:::note
**Sensitive Credentials:** When initializing a database, if the value `"sensitive"` is passed to specific fields in your `dolph_config.yaml`, the framework automatically checks your `.env` file for corresponding keys to use instead. This keeps your credentials securely out of source control.
- **MongoDB**: Passing `"sensitive"` to `url` pulls from `MONGO_URL`.
- **TypeORM & Sequelize**: Passing `"sensitive"` to `username`/`user`, `password`/`pass`, `host`, or `url` pulls from `SQL_USER`, `SQL_PASSWORD`, `SQL_HOST`, and `SQL_URL` respectively.
:::
