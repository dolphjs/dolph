...
The `@Component` decorator resolves every service it's given exactly once — the first time the component's module is imported — and caches that instance in a process-wide registry shared across the whole app. The DI container ensures that exactly one instance of `UserService` exists, promoting memory efficiency and making it easy to share state across the application if needed.

A controller can receive that instance one of two ways:

```typescript
// Constructor injection (recommended) — DolphJS resolves the parameter
// type and passes the singleton in automatically.
@Route('users')
...
```typescript
// Field injection — only applies when the controller has no constructor
// parameters. DolphJS assigns the instance to a property named after the
// service's class itself, i.e. `instance.UserService`, not `instance.userService`.
@Route('users')
export class UserController extends DolphControllerHandler<Dolph> {
...
See [Controllers → Dependency Injection](/core-concepts/controllers/#dependency-injection) for the full comparison, and [Testing](/testing/) for why constructor injection is the easier of the two to unit test.
